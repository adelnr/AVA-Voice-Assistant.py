...
talking = False  # Declare the variable outside of any functions
...
def ask_for_more():
    speak("Is there anything else I can do for you adel?")


#######web browser######
def google_search():
    # Open the web browser to the Google search page with the query
    speak("What do you want to search for adel?")
...
def conversationflow():
    while True:
        ...
        if "search" in said or "google" in said :
            google_search()
            ask_for_more()
...
def main():
    t1= threading.Thread(target=greeting_by_time)
    t2 = threading.Thread(target=conversationflow)
